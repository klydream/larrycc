...
namespace KingWoW
{
    class EnhancementShamanCombatClass : KingWoWAbstractBaseClass
    {
        ...
        float chainhealHopeRaidius = 13f;
        ...
        void BotEvents_OnBotStart(EventArgs args)
        {
            ...
            {
                utils.LogActivity("Glyph of Chaining Detected: range of chain heal set to 26");
                chainhealHopeRaidius = 26f;
            }
            else
            {
                chainhealHopeRaidius = 13f;
            }
            BotUpdate();
        }
        ...
        private bool Healing()
        {
            ...
            if (EnhancementShamanSettings.Instance.AssistHealing)
            {
                if (utils.AOEHealCount(Me, EnhancementShamanSettings.Instance.HealingTide_AncestralGuidanceHP, 40) >= EnhancementShamanSettings.Instance.HealingTide_AncestralGuidanceNumber)
                {
                    ...
                    if (utils.CanCast(HEALIG_TIDE_TOTEM) && !utils.isAuraActive(ANCESTRAL_GUIDANCE))
                    {
                ...
                HealingRain();
                ChainHeal();
                if (utils.CanCast(HEALING_STREAM_TOTEM) && !StyxWoW.Me.Totems.Any(t => t.WoWTotem == WoWTotem.HealingTide)
                    && utils.AOEHealCount(Me, EnhancementShamanSettings.Instance.HealingStreamHP, 40) >= EnhancementShamanSettings.Instance.HealingStreamNumber)
                {
        ...
        private bool Cleansing()
        {
            if (EnhancementShamanSettings.Instance.UseDispell)
            {
                ...
                if (player != null && player.Distance < 40 && player.InLineOfSpellSight)
                {
        ...
        private bool ChainHeal()
        {
            if (utils.PlayerCountBuff(MAELSTROM_WEAPON) == 5)
            {
                ...
                hr_target = utils.BestChainHealTarget(EnhancementShamanSettings.Instance.ChainHealHP, EnhancementShamanSettings.Instance.ChainHealNumber,chainhealHopeRaidius);

                if (hr_target != null)
                {
                    utils.LogActivity(CHAIN_HEAL, hr_target.Class.ToString());
                    return utils.Cast(CHAIN_HEAL,hr_target);
                }
            }
            return false;
        }
        ...
        private bool HealingRain()
        {
            if (utils.PlayerCountBuff(MAELSTROM_WEAPON) == 5)
            {
                ...
                hr_target = utils.BestHealingRainTargetLocation(EnhancementShamanSettings.Instance.HealingRainHP, EnhancementShamanSettings.Instance.HealingRainNumber);

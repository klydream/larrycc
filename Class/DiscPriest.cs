...
namespace KingWoW
{
    public class DiscPriestCombatClass : KingWoWAbstractBaseClass
    {
        ...
        {
            HotkeysManager.Register("Routine Pause", (Keys)DiscPriestSettings.Instance.PauseKey, DiscPriestSettings.Instance.ModKey, hk =>
            {
            ...
            HotkeysManager.Register("UseDisciplineAtonementHealingRotation", (Keys)DiscPriestSettings.Instance.UseDisciplineAtonementHealingRotationKey, DiscPriestSettings.Instance.ModKey, hk =>
            {
        ...
        private void RemoveHotkeys()
        {
            ...
            HotkeysManager.Unregister("UseDisciplineAtonementHealingRotation");
        }
        ...
        public override bool Pulse
        {
            get
            {
                ...
                if ((Me.Mounted && !DiscPriestSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !StyxWoW.IsInGame || !StyxWoW.IsInWorld || Me.Silenced || utils.IsGlobalCooldown(true) || Me.CastingSpell != null
                || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || Me.Auras.Any(x => x.Value.SpellId == SPIRIT_SHELL) || Me.Mounted || SpiritShell_detected())
                    return false;
                ...
                if (!Me.Combat && !Me.Mounted && DiscPriestSettings.Instance.OOCHealing && !utils.isAuraActive(DRINK) && !utils.isAuraActive(FOOD))
                {
                    ...
                    if (utils.CanCast(PENANCE) && hp <= PenancePercent && (!Me.IsMoving || (Me.IsMoving && talents.HasGlyph("Penance"))))
                    {
            ...
            get 
            {
                ...
                else if (target != null && !target.IsFriendly && target.Attackable && !target.IsDead)
                {
                    if (!target.InLineOfSpellSight || target.Distance - target.CombatReach -1  > DiscPriestSettings.Instance.PullDistance)
                    {
                        ...
                        movement.KingHealMove(target, DiscPriestSettings.Instance.PullDistance);
                    }
                    if (!Me.IsFacing(target) && (DiscPriestSettings.Instance.AutofaceTarget || SoloBotType) && !Me.IsMoving)
                    {
                        Me.SetFacing(target);
                    }
                    ...
                    {
                        utils.LogActivity("PULL: SHADOW_WORD_PAIN", target.Name);
                        return utils.Cast(SHADOW_WORD_PAIN, target);
                    }

                    if (utils.CanCast(SMITE,target,true) && !Me.IsMoving)
                    {
                        utils.LogActivity("PULL: SMITE", target.Name);
        ...
        private bool AoE()
        {
            ...
            {
                WoWPlayer pwb_target = utils.BestPWBTargetLocation(DiscPriestSettings.Instance.PWBarrierPercent, DiscPriestSettings.Instance.PWBarrierNumber);
                if (pwb_target != null)
                {
        ...
        private bool Atonement()
        {
            ...
            if (target != null)
            {
                if (ExtraUtilsSettings.Instance.movementEnabled && (target.Distance - target.CombatReach - 1 > DiscPriestSettings.Instance.PullDistance || !target.InLineOfSpellSight))
                    movement.KingHealMove(target, DiscPriestSettings.Instance.PullDistance);
                ...
                if (!talents.IsSelected(9) && utils.GetSpellCooldown(HOLY_FIRE).Milliseconds <= StyxWoW.WoWClient.Latency)
                {
        ...
        private bool Healing()
        {
            ...
            if (healTarget != null)
            {
                ...
                else
                {
                    ...
                    //binding Heal
                    if (healTarget.Guid != Me.Guid && hp <= BindingHealPercent && Me.HealthPercent <= BindingHealPercent
                    ...
                    if (hp <= DiscPriestSettings.Instance.PWShieldPercent && !utils.isAuraActive(POWER_WORD_SHIELD, healTarget)
                        && (!utils.isAuraActive(WEAKENED_SOUL, healTarget) || utils.isAuraActive(DIVINE_INSIGHT, Me)) && utils.CanCast(POWER_WORD_SHIELD,healTarget))
                    {
        ...
        {
            if (tank != null && tank.IsAlive && tank.Distance - tank.CombatReach -1  <= 40 && tank.InLineOfSpellSight)
            {
                ...
                if (tank.Combat && tank.HealthPercent <= DiscPriestSettings.Instance.TankShieldPercent && !utils.isAuraActive(POWER_WORD_SHIELD, tank)
                    && (!utils.isAuraActive(WEAKENED_SOUL, tank) || utils.isAuraActive(DIVINE_INSIGHT, Me)) && utils.CanCast(POWER_WORD_SHIELD,tank))
                {
        ...
        {
            if (off_tank != null && off_tank.IsAlive && off_tank.Distance - off_tank.CombatReach - 1 <= 40 && off_tank.InLineOfSpellSight)
            {
                ...
                if (off_tank.Combat && off_tank.HealthPercent <= DiscPriestSettings.Instance.OffTankShieldPercent && !utils.isAuraActive(POWER_WORD_SHIELD, off_tank)
                    && (!utils.isAuraActive(WEAKENED_SOUL, off_tank) || utils.isAuraActive(DIVINE_INSIGHT, Me)) && utils.CanCast(POWER_WORD_SHIELD,off_tank))
                {
       ...
       private bool CleansingASAP()
        ...
        private bool Dps()
        {
            ...
            else if (ExtraUtilsSettings.Instance.movementEnabled && Me.CurrentTarget != null && !Me.CurrentTarget.IsDead && (!Me.CurrentTarget.InLineOfSpellSight || Me.CurrentTarget.Distance - Me.CurrentTarget.CombatReach - 1 > DiscPriestSettings.Instance.PullDistance))
            {
        ...
        private bool ProcWork()
        {
            ...
            if (utils.isAuraActive(SURGE_OF_LIGHT, Me))
            {
                ...
                {
                    double hp = healTarget.HealthPercent;
                    if (healTarget.Distance -healTarget.CombatReach -1  > 40f || !healTarget.InLineOfSpellSight)
                    {
        ...
        public bool OnlyInstantSpellRotation()
        {
            ...
            if (healTarget != null)
            {
                ...
                else
                {
                    ...
                    if (!healTarget.HasAura(RENEW) && hp <= RenewPercent && utils.CanCast(RENEW))
                    {
                        utils.LogHealActivity(healTarget, " RENEW", healTarget.Class.ToString());
                        return utils.Cast(RENEW, healTarget);
                    }
        ...
        private bool Multidot()
        {
            if (DiscPriestSettings.Instance.MultidotEnabled)
            {
                    ...
                    /*|| enemyNumber >= DiscPriestSettings.Instance.Multidot_VampiricTouch_EnemyNumberMin*/)
                {
                    ...
                    {
                        TargetForMultidot = utils.NextApplyAuraTarget(SHADOW_WORD_PAIN, 40, 1500, DiscPriestSettings.Instance.MultidotAvoidCC);
        ...
        public override bool Combat
        {
            get
            {
                ...
                if ((Me.Mounted && !DiscPriestSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !Me.Mounted && !StyxWoW.IsInGame || !StyxWoW.IsInWorld || Me.Silenced || utils.IsGlobalCooldown(true) || utils.MeIsChanneling || Me.IsCasting
                || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) )
                    return false;

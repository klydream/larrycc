...
namespace KingWoW
{
    class ArcaneMageCombatClass : KingWoWAbstractBaseClass
    {
        ...
        {
            HotkeysManager.Register("Routine Pause", (Keys)ArcaneMageSettings.Instance.PauseKey, ArcaneMageSettings.Instance.ModKey, hk =>
            {
            ...
            HotkeysManager.Register("Multidot", (Keys)ArcaneMageSettings.Instance.MultidotKey, ArcaneMageSettings.Instance.ModKey, hk =>
            {
                ArcaneMageSettings.Instance.MultidotEnabled = !ArcaneMageSettings.Instance.MultidotEnabled;
            ...
            HotkeysManager.Register("AvoidAOE", (Keys)ArcaneMageSettings.Instance.AvoidAOEKey, ArcaneMageSettings.Instance.ModKey, hk =>
            {
         ...
         public override bool Combat
        {
            get
            {
                //avoid arcane blast with max charge if not any T16 proc
                if(ArcaneMageSettings.Instance.avoid_arcane_blast_at_max_charge && Me.IsCasting && utils.PlayerCountBuff(ARCANE_CHARGE) >= ArcaneMageSettings.Instance.CastBarrageAtCharge 
                    && utils.PlayerCountBuff(PROFOUND_MAGIC_T16_2P_BONUS) == 0 && Me.CastingSpell.Name.Equals(ARCANE_BLAST))
                    SpellManager.StopCasting();
                if ((Me.Mounted && !ArcaneMageSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !StyxWoW.IsInGame || !StyxWoW.IsInWorld || Me.Silenced/*|| utils.IsGlobalCooldown(true)*/ || Me.Silenced || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || Me.IsChanneling || utils.MeIsCastingWithLag())
                    return false;
        ...
        public override bool Pulse
        {
            get
            {
                ...
                if ((Me.Mounted && !ArcaneMageSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !StyxWoW.IsInGame || !StyxWoW.IsInWorld || Me.Silenced/*|| utils.IsGlobalCooldown(true)*/ || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || Me.IsChanneling || utils.MeIsCastingWithLag() || Me.Mounted)
                    return false;
        ...
        private bool PriorityBuff()
        {
            ...
            if (Me.Combat && utils.CanCast(TEMPORAL_SHIELD) && ArcaneMageSettings.Instance.AutoTemporalShield)
            {
        ...
        {
            if (utils.CanCast(FROST_BOMB) || utils.PlayerCountBuff(ARCANE_MISSILES_PROC) > 0 || utils.PlayerCountBuff(ARCANE_CHARGE) >= ArcaneMageSettings.Instance.CastBarrageAtCharge)
            {
                WoWUnit target = null;
                if (ArcaneMageSettings.Instance.TargetTypeSelected == ArcaneMageSettings.TargetType.MANUAL)
                    target = Me.CurrentTarget;
                ...
                if (target != null && !target.IsFriendly && target.Attackable && !target.IsDead && target.InLineOfSpellSight && target.Distance - target.CombatReach -1  <= 40)
                {
                    ...
                    if (utils.CanCast(ARCANE_BARRAGE,target)
                        ...
                        && utils.PlayerCountBuff(PROFOUND_MAGIC_T16_2P_BONUS) == 0)
                    {
                        utils.LogActivity(ARCANE_BARRAGE, target.Name);
                        return utils.Cast(ARCANE_BARRAGE, target);
                    }
                    if (!Me.IsMoving && utils.CanCast(ARCANE_BLAST, target) && utils.PlayerCountBuff(ARCANE_CHARGE) >= ArcaneMageSettings.Instance.CastBarrageAtCharge
                        && utils.PlayerCountBuff(PROFOUND_MAGIC_T16_2P_BONUS) >= 1)
                    {
                        utils.LogActivity("T16 2P BONUS:" + ARCANE_BLAST, target.Name);
                        return utils.Cast(ARCANE_BLAST, target);
                    }
        ...
        private bool CombatRotation()
        {
            ...
            if (target != null && !target.IsFriendly && target.Attackable && !target.IsDead && target.InLineOfSpellSight && target.Distance - target.CombatReach -1  <= 40)
            {
                ...
                if (utils.CanCast(NETHER_TEMPEST, target) && (utils.MyAuraTimeLeft(NETHER_TEMPEST, target) < 1500) && !(target.IsPlayer && ArcaneMageSettings.Instance.AvoidDOTPlayers))
                {
                ...
                if (utils.CanCast(LIVING_BOMB, target) && (utils.MyAuraTimeLeft(LIVING_BOMB, target) < 1500) && !(target.IsPlayer && ArcaneMageSettings.Instance.AvoidDOTPlayers))
                {
                ...
                //+++++++++++++++++++++++++AOE rotation start+++++++++++++++++++++++++++++++//
                if (ArcaneMageSettings.Instance.UseFlameStrike && utils.CanCast(FLAMESTRIKE) && !ArcaneMageSettings.Instance.AvoidAOE && target.Distance <= 40 && utils.AllAttaccableEnemyMobsInRangeFromTarget(target, 10).Count() >= ArcaneMageSettings.Instance.AOECount)
                {
                ...
                if (ArcaneMageSettings.Instance.UseArcaneExplosion && utils.CanCast(ARCANE_EXPLOSION) && !ArcaneMageSettings.Instance.AvoidAOE && utils.AllAttaccableEnemyMobsInRange(10).Count() >= ArcaneMageSettings.Instance.AOECount)
                {
            ...
            else if (ExtraUtilsSettings.Instance.movementEnabled && Me.CurrentTarget != null && !Me.CurrentTarget.IsDead && (!Me.CurrentTarget.InLineOfSpellSight || Me.CurrentTarget.Distance - Me.CurrentTarget.CombatReach - 1 > ArcaneMageSettings.Instance.PullDistance))
            {
        ...
        private bool UseCD()
        {
            if (Me.Combat && Me.GotTarget)
            {
                ...
                if (utils.CanCast(ALTER_TIME) && ArcaneMageSettings.Instance.CDUseAlterTime == ArcaneMageSettings.CDUseType.COOLDOWN
                    && (utils.PlayerCountBuff(ARCANE_CHARGE) >= ArcaneMageSettings.Instance.CastBarrageAtCharge && utils.PlayerCountBuff(ARCANE_MISSILES_PROC) >= 1))
                {
        ...
        private bool Multidot()
        {
            if (ArcaneMageSettings.Instance.MultidotEnabled)
            {
                int enemyNumber = utils.AllAttaccableEnemyMobsInRangeTargettingMyParty(40f, ArcaneMageSettings.Instance.MultidotAvoidCC).Count();
                if (enemyNumber >= ArcaneMageSettings.Instance.MultidotEnemyNumberMin)
                {
                    ...
                    {
                        TargetForMultidot = utils.NextApplyAuraTarget(NETHER_TEMPEST, 40, 1000, ArcaneMageSettings.Instance.MultidotAvoidCC, ArcaneMageSettings.Instance.AvoidDOTPlayers);
                    ...
                    {
                        TargetForMultidot = utils.NextApplyAuraTarget(LIVING_BOMB, 40, 1000, ArcaneMageSettings.Instance.MultidotAvoidCC, ArcaneMageSettings.Instance.AvoidDOTPlayers);
                        if (TargetForMultidot != null)
                        {

...
namespace KingWoW
{
    public class ShadowPriestCombatClass : KingWoWAbstractBaseClass
    {
        ...
        {
            WoWUnit target = Me.CurrentTarget;
            if (Me.Combat && (target == null || !target.Attackable ||target.IsFriendly || !target.InLineOfSpellSight || target.Distance - target.CombatReach -1  > RangeOfAttack || target.IsDead))
            {
                if (tank != null)
                    target = tank.CurrentTarget;
            }
            if (Me.Combat && target != null && target.Attackable && target.InLineOfSpellSight && target.Distance - target.CombatReach -1  <= RangeOfAttack && !target.IsDead)
            {
        ...
        private bool Multidot()
        {
            ...
            {
                int enemyNumber = utils.AllAttaccableEnemyMobsInRangeTargettingMyParty(40f, ShadowPriestSettings.Instance.MultidotAvoidCC).Count();
        ...
        {
            if (tank != null && tank.IsAlive && tank.Distance - tank.CombatReach - 1 <= 40 && tank.InLineOfSpellSight && (tank.Guid!=Me.Guid || Me.FocusedUnitGuid == Me.Guid))
            {
            ...
            else if (off_tank != null && off_tank.IsAlive && off_tank.Distance - off_tank.CombatReach - 1 <= 40 && off_tank.InLineOfSpellSight)
            {
        ...
        public bool CombatRotation()
        {
            ...
            if (target != null && !target.IsDead && target.InLineOfSpellSight && target.Distance - target.CombatReach - 1 <= 40)
            {
                ...
                if (!Me.IsMoving && Me.GetCurrentPower(WoWPowerType.ShadowOrbs) < 3 && talents.IsSelected(9) && utils.MyAuraTimeLeft(DEVOURING_PLAGUE, target) > 0 /*&& utils.CanCast(MIND_FLY, target)*/
                    /*&& nextChanneledMFSpellAllowed <= DateTime.Now*/ && Me.CurrentChannelTimeLeft.Milliseconds <= 500)
                {
        ...
        public override bool Combat
        {
            get
            {
                if (/*ExtraUtilsSettings.Instance.PauseRotation || */ !StyxWoW.IsInGame || !StyxWoW.IsInWorld /*|| utils.IsGlobalCooldown(true)*/ || Me.Silenced || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || (Me.IsChanneling && Me.CurrentChannelTimeLeft.Milliseconds >= 500) || utils.MeIsCastingWithLag())
                    return false;

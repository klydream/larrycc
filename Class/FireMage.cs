...
namespace KingWoW
{
    class FireMageCombatClass : KingWoWAbstractBaseClass
    {
        ...
        {
            HotkeysManager.Register("Routine Pause", (Keys)FireMageSettings.Instance.PauseKey, FireMageSettings.Instance.ModKey, hk =>
            {
            ...
            HotkeysManager.Register("Multidot", (Keys)FireMageSettings.Instance.MultidotKey, FireMageSettings.Instance.ModKey, hk =>
            {
                FireMageSettings.Instance.MultidotEnabled = !FireMageSettings.Instance.MultidotEnabled;
            ...
            HotkeysManager.Register("AvoidAOE", (Keys)FireMageSettings.Instance.AvoidAOEKey, FireMageSettings.Instance.ModKey, hk =>
            {
        ...
        public override bool Combat
        {
            get
            {
                if ((Me.Mounted && !FireMageSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !StyxWoW.IsInGame || !StyxWoW.IsInWorld || Me.Silenced/*|| utils.IsGlobalCooldown(true)*/ || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || Me.IsChanneling || utils.MeIsCastingWithLag())
                    return false;
        ...
        public override bool Pulse
        {
            get
            {
                ...
                if ((Me.Mounted && !FireMageSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !StyxWoW.IsInGame || !StyxWoW.IsInWorld || Me.Silenced/*|| utils.IsGlobalCooldown(true)*/ || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || Me.IsChanneling || utils.MeIsCastingWithLag() || Me.Mounted)
                    return false;
        ...
        private bool PriorityBuff()
        {
            ...
            if (Me.Combat && utils.CanCast(TEMPORAL_SHIELD) && FireMageSettings.Instance.AutoTemporalShield)
            {
        ...
        private bool CombatRotation()
        {
            ...
            if (target != null && !target.IsFriendly && target.Attackable && !target.IsDead && target.InLineOfSpellSight && target.Distance - target.CombatReach -1  <= 40)
            {
                ...
                if (utils.CanCast(NETHER_TEMPEST, target) && (utils.MyAuraTimeLeft(NETHER_TEMPEST, target) < 1500) && !(target.IsPlayer && FireMageSettings.Instance.AvoidDOTPlayers))
                {
                ...
                if (utils.CanCast(LIVING_BOMB, target) && (utils.MyAuraTimeLeft(LIVING_BOMB, target) < 1500) && !(target.IsPlayer && FireMageSettings.Instance.AvoidDOTPlayers))
                {
                ...
                //+++++++++++++++++++++++++AOE rotation start+++++++++++++++++++++++++++++++//
                if (FireMageSettings.Instance.UseFlameStrike && utils.CanCast(FLAMESTRIKE) && !FrostMageSettings.Instance.AvoidAOE && target.Distance <= 40 && utils.AllAttaccableEnemyMobsInRangeFromTarget(target, 10).Count() >= FireMageSettings.Instance.AOECount)
                {
                ...
                if (FireMageSettings.Instance.UseDragonBreath && utils.CanCast(DRAGON_BREATH) && !FrostMageSettings.Instance.AvoidAOE && utils.AllAttaccableEnemyMobsInRange(15).Count() >= FireMageSettings.Instance.AOECount)
                {
                    utils.LogActivity(DRAGON_BREATH);
                    return utils.Cast(DRAGON_BREATH);
                }

                if (FireMageSettings.Instance.UseArcaneExplosion && utils.CanCast(ARCANE_EXPLOSION) && !FrostMageSettings.Instance.AvoidAOE && utils.AllAttaccableEnemyMobsInRange(10).Count() >= FireMageSettings.Instance.AOECount)
                {
            ...
            else if (ExtraUtilsSettings.Instance.movementEnabled && Me.CurrentTarget != null && !Me.CurrentTarget.IsDead && (!Me.CurrentTarget.InLineOfSpellSight || Me.CurrentTarget.Distance - Me.CurrentTarget.CombatReach - 1 > FireMageSettings.Instance.PullDistance))
            {
        ...
        private bool Multidot()
        {
            if (FireMageSettings.Instance.MultidotEnabled)
            {
                int enemyNumber = utils.AllAttaccableEnemyMobsInRangeTargettingMyParty(40f, FireMageSettings.Instance.MultidotAvoidCC).Count();
                if (enemyNumber >= FireMageSettings.Instance.MultidotEnemyNumberMin)
                {
                    ...
                    {
                        TargetForMultidot = utils.NextApplyAuraTarget(NETHER_TEMPEST, 40, 1000, FireMageSettings.Instance.MultidotAvoidCC, FireMageSettings.Instance.AvoidDOTPlayers);
                    ...
                    {
                        TargetForMultidot = utils.NextApplyAuraTarget(LIVING_BOMB, 40, 1000, FireMageSettings.Instance.MultidotAvoidCC, FireMageSettings.Instance.AvoidDOTPlayers);
                        if (TargetForMultidot != null)
                        {

    ...
    public class ProtPaladinCombatClass : KingWoWAbstractBaseClass
    {
        ...
        public override bool Pulse
        {
            get
            {
                ...
                || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || Me.Mounted)
                    return false;
                ...
                {
                    Resurrect();
                    if (ProtPaladinSettings.Instance.OutOfCombatHeal)
                    {
        ...
        private bool ProcHandler()
        {
            ...
            //Flash of Light free
            if (ProtPaladinSettings.Instance.AutoHeal && ProtPaladinSettings.Instance.UseHealingPartyMember && StyxWoW.Me.ActiveAuras.ContainsKey(SELFLESS_HEALER) && utils.GetAuraStack(Me, SELFLESS_HEALER, true) == 3)
            {
                WoWUnit healTarget = utils.GetHealTarget(40f);
                if (healTarget != null && healTarget.HealthPercent <= ProtPaladinSettings.Instance.ProcAutoHealHPSelfless)
                {
        ...
        private bool SoloRotation()
        {
            ...
            {
                if (Me.CurrentHolyPower >= 3 && Me.HealthPercent <= ProtPaladinSettings.Instance.HolyPowerAutoHealHP_EF_WoG)
                {
                    if (utils.CanCast(ETERNAL_FLAME))
                    {
                        {
            ...
            if (ProtPaladinSettings.Instance.UseHealingPartyMember)
            {
                ...
                {
                    if (Me.CurrentHolyPower >= 3 && healTarget.HealthPercent <= ProtPaladinSettings.Instance.HolyPowerAutoHealHP_EF_WoG)
                    {
                        if (utils.CanCast(ETERNAL_FLAME))
                        {
                    ...
                    if (healTarget.HealthPercent <= ProtPaladinSettings.Instance.AutoHealTier6)
                    {
                        if (utils.CanCast(EXECUTION_SENTENCE))
                        {
                            utils.LogActivity(EXECUTION_SENTENCE, healTarget.Class.ToString());
                            return utils.Cast(EXECUTION_SENTENCE, healTarget);
                        }
                        if (utils.CanCast(HOLY_PRISM))
                        {
                            utils.LogActivity(HOLY_PRISM, healTarget.Class.ToString());
                            return utils.Cast(HOLY_PRISM, healTarget);
                        }
                        if (utils.CanCast(LIGHTS_HAMMER))
                        {
                            utils.LogActivity(LIGHTS_HAMMER, healTarget.Class.ToString());
                            utils.Cast(LIGHTS_HAMMER);
                            return SpellManager.ClickRemoteLocation(healTarget.Location);
                        }
                    }
                }
            }

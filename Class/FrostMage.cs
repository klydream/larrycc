...
namespace KingWoW
{
    class FrostMageCombatClass : KingWoWAbstractBaseClass
    {
        ...
        private WoWUnit flamestrikeCandidateTarget = null;
        ...
        {
            HotkeysManager.Register("Routine Pause", (Keys)FrostMageSettings.Instance.PauseKey, FrostMageSettings.Instance.ModKey, hk =>
            {
            ...
            HotkeysManager.Register("Multidot", (Keys)FrostMageSettings.Instance.MultidotKey, FrostMageSettings.Instance.ModKey, hk =>
            {
                FrostMageSettings.Instance.MultidotEnabled = !FrostMageSettings.Instance.MultidotEnabled;
            ...
            HotkeysManager.Register("AvoidAOE", (Keys)FrostMageSettings.Instance.AvoidAOEKey, FrostMageSettings.Instance.ModKey, hk =>
            {
        ...
        public override bool Combat
        {
            get 
            {
                ...
                {
                    int time = utils.SpecificEncounterNeedStopCast();
                    if (time > 0)
                    {
                ...
                if ((Me.Mounted && !FrostMageSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !StyxWoW.IsInGame || !StyxWoW.IsInWorld /*|| utils.IsGlobalCooldown(true)*/ || Me.Silenced || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || Me.IsChanneling || utils.MeIsCastingWithLag())
                    return false;
        ...
        public override bool Pulse
        {
            get 
            {
                ...
                if ((Me.Mounted && !FrostMageSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !StyxWoW.IsInGame || !StyxWoW.IsInWorld || Me.Silenced /*|| utils.IsGlobalCooldown(true)*/ || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD) || Me.IsChanneling || utils.MeIsCastingWithLag() || Me.Mounted)
                    return false;
        ...
        private bool PriorityBuff()
        {
            ...
            if (FrostMageSettings.Instance.EvocationBuffAuto && !Me.IsMoving && talents.IsSelected(16) && (!utils.isAuraActive(INVOCATION_BUFF) || Me.ManaPercent <= 5) && utils.CanCast(EVOCATION))
            {
                utils.LogActivity(EVOCATION);
                return utils.Cast(EVOCATION);
            }
            ...
            if (Me.Combat && utils.CanCast(TEMPORAL_SHIELD) && FrostMageSettings.Instance.AutoTemporalShield)
            {
        ...
        private bool ProcWork()
        {
            ...
            if (utils.CanCast(FROST_BOMB))
            {
                ...
                if (target != null && !target.IsDead && target.InLineOfSpellSight && target.Distance - target.CombatReach - 1 <= 40 && !(target.IsPlayer && FrostMageSettings.Instance.AvoidDOTPlayers))
                {
        ...
        private bool OnlyInstantSpellRotation()
        {
            ...
            if (target != null && !target.IsDead && target.InLineOfSpellSight && target.Distance - target.CombatReach - 1 <= 40)
            {
                ...
                if (utils.CanCast(NETHER_TEMPEST, target) && (utils.MyAuraTimeLeft(NETHER_TEMPEST, target) < 1500) && !(target.IsPlayer && FrostMageSettings.Instance.AvoidDOTPlayers))
                {
                ...
                if (utils.CanCast(LIVING_BOMB, target) && (utils.MyAuraTimeLeft(LIVING_BOMB, target) < 1500) && !(target.IsPlayer && FrostMageSettings.Instance.AvoidDOTPlayers))
                {
                ...
                if (FrostMageSettings.Instance.AutoFrozerOrb && utils.CanCast(FROZEN_ORB, target) && !FrostMageSettings.Instance.AvoidAOE)
                {
                ...
                //+++++++++++++++++++++++++AOE rotation start+++++++++++++++++++++++++++++++//
                if (FrostMageSettings.Instance.UseConeOfCold && utils.CanCast(CONE_OF_COLD) && !FrostMageSettings.Instance.AvoidAOE && utils.FacingEnemy(12).Count() >= FrostMageSettings.Instance.ConeOfColdCount)
                {
                    utils.LogActivity(CONE_OF_COLD);
                    utils.Cast(CONE_OF_COLD);
                }
                if (FrostMageSettings.Instance.UseArcaneExplosion && utils.CanCast(ARCANE_EXPLOSION) && !FrostMageSettings.Instance.AvoidAOE && utils.AllAttaccableEnemyMobsInRange(10).Count() >= FrostMageSettings.Instance.ExplosionAOECount)
                {
                ...
                if (FrostMageSettings.Instance.UsePetFreeze && utils.CanCastPetAction(FREEZE) && target.Distance /*- target.CombatReach - 1*/ <= 35
                    && !target.IsBoss && !FrostMageSettings.Instance.AvoidAOE)
                {
        ...
        private bool CombatRotation()
        {
            ...
            if (target != null && !target.IsDead && target.InLineOfSpellSight && target.Distance - target.CombatReach - 1 <= 40)
            {
                ...
                if (FrostMageSettings.Instance.UseFlameStrike && utils.CanCast(FLAMESTRIKE) && !FrostMageSettings.Instance.AvoidAOE)// && target.Distance - target.CombatReach - 1 <= 40 && utils.AllAttaccableEnemyMobsInRangeFromTarget(target, 10).Count() >= FrostMageSettings.Instance.AOECount)
                {
                    flamestrikeCandidateTarget = utils.EnemyInRangeWithMobsAround(40, 10, FrostMageSettings.Instance.FlamestrikeAOECount);
                    if (flamestrikeCandidateTarget != null)
                    {
                        utils.LogActivity(FLAMESTRIKE, flamestrikeCandidateTarget.Name);
                ...
                {
                    blizzardCandidateTarget = utils.EnemyInRangeWithMobsAround(35, 10, FrostMageSettings.Instance.BlizzardAOECount);
                    if (blizzardCandidateTarget != null)
                    {
                        utils.LogActivity(BLIZZARD, blizzardCandidateTarget.Name);
                        utils.Cast(BLIZZARD);
            ...
            else if (ExtraUtilsSettings.Instance.movementEnabled && Me.CurrentTarget != null && !Me.CurrentTarget.IsDead && (!Me.CurrentTarget.InLineOfSpellSight || Me.CurrentTarget.Distance - Me.CurrentTarget.CombatReach - 1 > FrostMageSettings.Instance.PullDistance))
            {
        ...
        private bool Multidot()
        {
            if (FrostMageSettings.Instance.MultidotEnabled)
            {
                int enemyNumber = utils.AllAttaccableEnemyMobsInRangeTargettingMyParty(40f, FrostMageSettings.Instance.MultidotAvoidCC).Count();
                if (enemyNumber >= FrostMageSettings.Instance.MultidotEnemyNumberMin)
                {
                    ...
                    {
                        TargetForMultidot = utils.NextApplyAuraTarget(NETHER_TEMPEST, 40, 1000, FrostMageSettings.Instance.MultidotAvoidCC,FrostMageSettings.Instance.AvoidDOTPlayers);
                    ...
                    {
                        TargetForMultidot = utils.NextApplyAuraTarget(LIVING_BOMB, 40, 1000, FrostMageSettings.Instance.MultidotAvoidCC,FrostMageSettings.Instance.AvoidDOTPlayers);
                        if (TargetForMultidot != null)
                        {

...
namespace KingWoW
{
    public class HolyPriestCombatClass : KingWoWAbstractBaseClass
    {
        ...
        {
            HotkeysManager.Register("Routine Pause", (Keys)HolyPriestSettings.Instance.PauseKey, HolyPriestSettings.Instance.ModKey, hk =>
            {
        ...
        public override bool Pulse
        {
            get
            {
                ...
                if (HolyPriestSettings.Instance.TrySaveMana)
                    StopCastingCheck();
                if ((Me.Mounted && !HolyPriestSettings.Instance.AutoDismountOnCombat) || IsCRPaused || !StyxWoW.IsInGame || !StyxWoW.IsInWorld /*|| utils.IsGlobalCooldown(true)*/ || utils.MeIsChanneling || Me.IsCasting || Me.Mounted
        ...
        public override bool Combat
        {
            ...
            {               
                    if (HolyPriestSettings.Instance.TrySaveMana)
                        StopCastingCheck();
                    if ((Me.Mounted && !HolyPriestSettings.Instance.AutoDismountOnCombat) || IsCRPaused  || !Me.Mounted && !StyxWoW.IsInGame || !StyxWoW.IsInWorld /*|| utils.IsGlobalCooldown(true)*/ || utils.MeIsChanneling || Me.IsCasting
                    || utils.isAuraActive(DRINK) || utils.isAuraActive(FOOD))
                        return false;
        ...
        public override bool Pull
        {
            get
            {
                ...
                if (target != null && !target.IsFriendly && target.Attackable && !target.IsDead)
                {
                    ...
                    {
                        if (!Me.IsFacing(target) && (HolyPriestSettings.Instance.AutofaceTarget || SoloBotType) && !Me.IsMoving)
                        {
        ...
        {
            WoWUnit target = Me.CurrentTarget;
            if (Me.Combat && (target == null || !target.Attackable || target.IsFriendly || !target.InLineOfSpellSight || target.Distance > 40 || target.IsDead))
            {
                if (tank != null)
                    target = tank.CurrentTarget;
            }
            if (Me.Combat && target != null && target.Attackable && target.InLineOfSpellSight && target.Distance < 40 && !target.IsDead)
            {
        ...
        private bool Healing()
        {
            ...
            if (Me.CurrentTarget != null && Me.CurrentTarget.IsFriendly && Me.CurrentTarget.IsValid
                ...
                && healTarget.HealthPercent > HolyPriestSettings.Instance.HealNPC_threshold)
            ...
            if (healTarget != null)
            {
                ...
                if (healTarget.Distance > 40 || !healTarget.InLineOfSpellSight)
                {
        ...
        private bool Cleansing()
        {
            if (HolyPriestSettings.Instance.UsePurify)
            {
                ...
                if (player != null && !Blacklist.Contains(player.Guid, BlacklistFlags.All) && player.Distance < 40 && player.InLineOfSpellSight)
                {
        ...
        private bool Dps()
        {
            ...
            if (target != null)
            {
                ...
                if (ExtraUtilsSettings.Instance.movementEnabled && (!target.InLineOfSpellSight || target.Distance > HolyPriestSettings.Instance.PullDistance))
                {
                    movement.KingHealMove(target, HolyPriestSettings.Instance.PullDistance);
                }
            }
        ...
        private bool CastHW_Sanctuary(bool lockframe)
        {
            ...
            if (!Me.IsMoving && !WoWSpell.FromId(88685).Cooldown && utils.isAuraActive(CHAKRA_SANCTUARY,Me))
            {
                ...
                {
                    hws_target = utils.BestHWSanctuaryTargetLocation(HolyPriestSettings.Instance.HWSanctuaryPercent, HolyPriestSettings.Instance.HWSanctuaryNumber);
                }
                if (hws_target != null)
                {

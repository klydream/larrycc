...
namespace KingWoW
{
    public class KingWoWUtility
    {
        ...
        #region ENCOUNTERS_SPECIFIC
        
        //BOSS
        public const int Malkorok = 71454;
        public const int Thok_the_Bloodthirsty = 71529;
        public const int OOndasta = 69161;
        public const int Dark_Animus = 69427;


        
        public WoWUnit CurrentBossInRange(double range=200)
        {
            return AllEnemyMobs.Where(u =>
                        (u.Distance - u.CombatReach -1  <= range && u.IsBoss && u.IsAlive)).FirstOrDefault();
        }

        public int SpecificEncounterNeedStopCast()
        {
            WoWUnit TheBoss = CurrentBossInRange();
            double MycurrentTimeCastLeft = 0;
            if (TheBoss != null && TheBoss.IsCasting && (Me.IsCasting || Me.IsChanneling) )
            {
                if(Me.IsChanneling)
                    MycurrentTimeCastLeft = Me.CurrentChannelTimeLeft.Milliseconds + StyxWoW.WoWClient.Latency;
                else if (Me.IsCasting)
                    MycurrentTimeCastLeft = Me.CurrentCastTimeLeft.Milliseconds + StyxWoW.WoWClient.Latency;
                switch (TheBoss.Entry)
                {
                    case Thok_the_Bloodthirsty:
                        if (TheBoss.CastingSpellId == 143343 /*DeafeningScreech*/ && TheBoss.CurrentCastTimeLeft.Milliseconds < MycurrentTimeCastLeft)
                            return TheBoss.CurrentCastTimeLeft.Milliseconds;
                        break;
                    case OOndasta:
                        if (TheBoss.CastingSpellId == 137457 /*PiercingRoar*/  && TheBoss.CurrentCastTimeLeft.Milliseconds < MycurrentTimeCastLeft)
                            return TheBoss.CurrentCastTimeLeft.Milliseconds;
                        break;
                }
            }
        ...
        public WoWUnit EnemyInRangeWithMobsAround(double range, double radius, int count)
        {
            return AllAttaccableEnemyMobsInRangeTargettingMyParty(range,false).Where(u =>
                        (u.Distance - u.CombatReach - 1 <= range && AllAttaccableEnemyMobsInRangeFromTarget(u, radius).Count() >= count)).FirstOrDefault();
        }

        public IEnumerable<WoWUnit> AllAttaccableEnemyMobsInRangeTargettingMyParty(double range, bool avoidCC=false)
        {         
            if(avoidCC)
                return AllEnemyMobs.Where(u =>
        ...
        {
            return AllAttaccableEnemyMobsInRangeTargettingMyParty(range, avoidCC).OrderBy(uu => uu.Distance).FirstOrDefault();
        }

        public WoWUnit NearestAttaccableEnemyInRangePVPTargettingMyParty(double range, bool avoidCC = false)
        {
            return AllAttaccableEnemyMobsInRangeTargettingMyParty(range, avoidCC).Where(uu => !healInterrDispClea_Manager.InvulnerableTarget(uu) && !healInterrDispClea_Manager.InvulnerableToSpellTarget(uu)).OrderBy(uu => uu.Distance).FirstOrDefault();
        }
        ...
        //string
        public WoWUnit NextApplyAuraTarget(string aura, double range, double timeToRefresh, bool avoidCC, bool avoidplayer=false)
        {
            WoWUnit target = null;
            if(avoidplayer)
                target = (from unit in AllAttaccableEnemyMobsInRangeTargettingMyParty(range, avoidCC)
        ...
        {
            WoWUnit target = (from unit in AllAttaccableEnemyMobsInRangeTargettingMyParty(range, avoidCC)
                              //where unit.ThreatInfo.RawPercent < 90
        ...
        public bool NeedsDispelShammyNotResto(WoWUnit unit)
        {
            if (healInterrDispClea_Manager.TargetDontDispell(unit))
                return false;
            if (unit.ActiveAuras.Any(a => a.Value.IsHarmful && a.Value.Spell.DispelType == WoWDispelType.Curse))
        ...
        public WoWPlayer GetDispellTargetShammyNotResto(double range)
        {
            return (from unit in ObjectManager.GetObjectsOfTypeFast<WoWPlayer>()
                    orderby unit.HealthPercent ascending
                    where (!unit.IsDead
                    ...
                    && unit.Distance - unit.CombatReach - 1 <= range
                    && unit.InLineOfSpellSight
                    && NeedsDispelShammyNotResto(unit)
                    && (unit.IsInMyPartyOrRaid || unit.IsMe))
                    select unit).FirstOrDefault();
        }
        ...
        public float GetDistance(WoWPoint p1, WoWPoint p2)
        {
            return p1.Distance(p2);
        }

        public double AOEHealCount(WoWPlayer player, int hp, float dist)
        {
            if (player == null)
                return 0;
            return GroupMembers.Count(
                unit =>
                    ...
                    unit.ToPlayer().HealthPercent <= hp &&
                    GetDistance(player.Location, unit.ToPlayer().Location) <= dist);
        ...
        public int GetPartyAOECount(WoWPartyMember player, int hp)
        {
            ...
            return Enumerable.Count((from p in GroupMembers
                                    ...
                                    && !healInterrDispClea_Manager.TargetDontHeal(p.ToPlayer())
        ...
        public IEnumerable<WoWUnit> FacingEnemy(float range)
        {
            return AllEnemyMobs.Where(u =>
                        (u.Distance - u.CombatReach - 1 <= range && u.InLineOfSpellSight && u.Attackable && Me.IsFacing(u)) );
        }
        ...
        public IEnumerable<WoWUnit> FacingUnit(float range, int hp, bool friendly, bool enemy)
        {
            if (friendly && enemy)
                ...
                && o.ToUnit().Distance - o.ToUnit().CombatReach -1  <= range && o.ToUnit().InLineOfSpellSight && Me.IsSafelyFacing(o,45f)).Select(o => o.ToUnit()).ToList();
        ...
        {
            WoWPartyMember PlayerCandidate = GroupMembers.FirstOrDefault(
                                                         unit =>
                                                             unit.ToPlayer() != null &&
                                                             unit.ToPlayer().Distance <= 40 &&
                                                             unit.ToPlayer().InLineOfSpellSight &&
                                                             unit.ToPlayer().IsAlive &&
                                                             unit.ToPlayer().HealthPercent <= hp &&
                                                             AOEHealCount(unit.ToPlayer(), hp, PW_BARRIER_RADIUS) >= count);
        ...
        public WoWPlayer BestHealingRainTargetLocation(int hp, int count)
        {
            WoWPartyMember PlayerCandidate = GroupMembers.FirstOrDefault(
                                                         unit =>
                                                             unit.ToPlayer() != null &&
                                                             ...
                                                             unit.ToPlayer().IsAlive &&
                                                             unit.ToPlayer().HealthPercent <= hp &&
                                                             AOEHealCount(unit.ToPlayer(), hp, HEALING_RAIN_Radius()) >= count);
            if (PlayerCandidate != null)
                return PlayerCandidate.ToPlayer();
            else
                return null;
        ...
        public WoWPlayer BestChainHealTarget(int hp, int count, float chainHopeDistance)
        {
            WoWPartyMember PlayerCandidate = GroupMembers.FirstOrDefault(
                                                         unit =>
                                                             unit.ToPlayer() != null &&
                                                             ...
                                                             unit.ToPlayer().IsAlive &&
                                                             unit.ToPlayer().HealthPercent <= hp &&
                                                             AOEHealCount(unit.ToPlayer(), hp, chainHopeDistance) >= count);
            if (PlayerCandidate != null)
                return PlayerCandidate.ToPlayer();
            else
                return null;
        ...
        public WoWPlayer BestHWSanctuaryTargetLocation(int hp, int count)
        {
            WoWPartyMember PlayerCandidate = GroupMembers.FirstOrDefault(
                                                         unit =>
                                                             unit.ToPlayer() != null &&
                                                             ...
                                                             unit.ToPlayer().IsAlive &&
                                                             unit.ToPlayer().HealthPercent <= hp &&
                                                             AOEHealCount(unit.ToPlayer(), hp, HW_SANCTUARY_RADIUS) >= count);
            if (PlayerCandidate != null)
                return PlayerCandidate.ToPlayer();
            else
                return null;
        ...
        public WoWPlayer BestPOM_PROC_DIVINE_INSIGHT_Target(int hp, int count)
        {
            WoWPartyMember PlayerCandidate = GroupMembers.FirstOrDefault(
                                                         unit =>
                                                             ...
                                                             unit.ToPlayer().IsAlive &&
                                                             unit.ToPlayer().HealthPercent <= hp &&
                                                             AOEHealCount(unit.ToPlayer(), hp, POM_Radius()) >= count);
            if (PlayerCandidate != null)
                return PlayerCandidate.ToPlayer();
            else
                return null;
        ...
        public WoWPlayer BestCircleOfHealing_Target(int hp, int count)
        {
            WoWPartyMember PlayerCandidate = GroupMembers.FirstOrDefault(
                                                         unit =>
                                                             ...
                                                             unit.ToPlayer().IsAlive &&
                                                             unit.ToPlayer().HealthPercent <= hp &&
                                                             AOEHealCount(unit.ToPlayer(), hp, COHRadius()) >= count);
            if (PlayerCandidate != null)
                return PlayerCandidate.ToPlayer();
            else
                return null;
        ...
        {
            WoWUnit PlayerCandidate = null;
            if(AOEHealCount(Me, hp, CHAIN_HEAL_Radius() ) >= count)
                PlayerCandidate = GetHealTarget(40f);
            if (PlayerCandidate != null)
                return PlayerCandidate.ToPlayer();
            else
                return null;
        ...
        //TIME TO DEATH TY to singular developer!!
        private ulong guid { get; set; }  // guid of mob
        private uint _firstLife;         // life of mob when first seen
        private uint _firstLifeMax;      // max life of mob when first seen
        private int _firstTime;          // time mob was first seen
        private uint _currentLife;       // life of mob now
        private int _currentTime;        // time now

        private static DateTime timeOrigin = new DateTime(2012, 1, 1); // Refernzdatum (festgelegt)
